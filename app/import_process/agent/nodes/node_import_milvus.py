...
def step_2_prepare_collection():
    ...
    # 2. 判断是否存在,不存在进行创建流程
    if not mivlus_client.has_collection(collection_name=milvus_config.chunks_collection):
        # 创建schema filed
        schema = mivlus_client.create_schema(
            auto_id=True,
        ...
        # 3. 创建列对应的索引
        index_params = mivlus_client.prepare_index_params()
        ...
        # 4. 创建集合collection
        mivlus_client.create_collection(
