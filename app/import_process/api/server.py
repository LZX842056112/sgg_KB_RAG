...
@app.get("/import/html")  # url:     /import/html
...
async def upload(backgroundtasks: BackgroundTasks, files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        # 保存文件
        # 读取文件字节 , file.file是SpooledTemporaryFile 不能直接写出! 否则出现异常!
        file_bytes = await file.read()
        # file.file是SpooledTemporaryFile
        local_file_path_obj.write_bytes(file_bytes)
        # 3.拼接state属性 [task_id,local_file_path,local_dir]
        # 4.异步调用 import_graph_app . invoke(state) -> 8个节点....
